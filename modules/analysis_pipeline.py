...
from .profiling_module import AnalysisPlan, validate_plan
from .ui_components import display_analysis_plan, display_plan_review_interface, feedback_input_interface
from .categorize import extract_keywords_parallel, categorize_keywords_batch, generate_wordcloud_from_freq
from .sentiment_module import analyze_sentiment, plot_sentiment_distribution
from .summary_module import generate_summary_with_gpt
...
class AnalysisPipeline:
    ...
    def step1_generate_plan(self, data: pd.DataFrame, description: str):
        ...
        with col2:
            ...
            st.write(f"- 컬럼 수: {len(data.columns)}")
    ...
    def step2_review_plan(self):
        ...
        with col3:
            if st.button("✅ 기획서 승인", type="primary", use_container_width=True) and validation_passed:
    ...
    def _run_analysis(self, data: pd.DataFrame, plan: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            #     keyword_result = self._run_keyword_analysis(target_data, modules["keyword_analysis"]["target_columns"])
            ...
            # # 감정 분석
            # if modules["sentiment_analysis"]["use"]:
            #     status_text.text("😊 감정 분석 중...")
            #     sentiment_result = self._run_sentiment_analysis(target_data, modules["sentiment_analysis"]["target_columns"])
            ...
            # # 요약 분석
            # if modules["summary_analysis"]["use"]:
            #     status_text.text("📝 요약 분석 중...")
            #     summary_result = self._run_summary_analysis(target_data, modules["summary_analysis"]["target_columns"])
            #     results["summary_analysis"] = summary_result
            #     current_step += 1
            #     progress_bar.progress(current_step / total_steps)
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                if modules["keyword_analysis"]["use"]:
                    futures["keyword"] = executor.submit(
                        self._run_keyword_analysis_fast,
        ...
        try:
            ...
            for col in target_columns:
                if col in data.columns:
                    ...
                    progress_container.write(f"   - {col}: {len(valid_texts):,}개 텍스트 수집")
        ...
        try:
            ...
            for i, text in enumerate(texts):
                try:
                    ...
                    # 실패율이 50% 이상이면 중단
                    if failed_count > len(texts) * 0.5:
                        return {
                            "error": f"감정 분석 실패율이 너무 높습니다. 실패: {failed_count}/{len(texts)}",
                            "texts_analyzed": i+1,
                            "failed_count": failed_count
                        }
            
            if not sentiments:
                return {"error": "감정 분석 결과가 없습니다.", "texts_analyzed": len(texts)}
        ...
        if st.button("🚀 AI 분석 시작", type="primary", use_container_width=True):
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                if modules.get("keyword_analysis", {}).get("use", False):
                    futures["keyword"] = executor.submit(self._run_keyword_analysis_fast, data, target_columns)
